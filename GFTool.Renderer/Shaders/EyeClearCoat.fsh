...
vec2 WrapUv01(vec2 uv)
{
    ...
    // Many TRMTR samplers for these maps are CLAMP, so explicitly wrap into [0,1]
    return fract(uv);
}
...
vec4 GetLayerMask(vec2 uv)
{
    ...
    // If scale params are missing, default to 1 (uniform default is 0 at link time)
...
void main()
{
    ...
    vec3 specLit = (spec + clearCoat) * vec3(0.8) * SpecularScale;
